...
class WorldManager {
    ...
         // Get the highest solid block Y coordinate at a given X, Z position
         // Used for initial player placement
         getHighestSolidBlockY(x, z) {
             // Convert physical X, Z to world data X, Z
             const worldDataX = Math.floor(x + this.worldWidth / 2);
             const worldDataZ = Math.floor(z + this.worldDepth / 2);
    
            if (worldDataX < 0 || worldDataX >= this.worldWidth || worldDataZ < 0 || worldDataZ >= this.worldDepth) {
                return 0; // Return base level if out of bounds
            }
    
            // Search downwards from the top of the world
            for (let y = this.worldHeight - 1; y >= 0; y--) {
                if (this.getBlock(worldDataX, y, worldDataZ) !== null) {
                    return y; // Return the y coordinate of the highest solid block
                }
            }
            return 0; // Return 0 if no solid block found (only air/bedrock)
         }
